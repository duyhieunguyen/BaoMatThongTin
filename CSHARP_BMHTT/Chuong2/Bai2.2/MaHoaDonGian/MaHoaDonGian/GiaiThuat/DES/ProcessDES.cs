...
namespace MaHoaDonGian.GiaiThuat.DES
{
    ...
    {
        #region Các biến cố
         public frmMaHoaGiaiMa.ProgressInitHandler InitProgress;
         public frmMaHoaGiaiMa.ProgressEventArgs IncrementProgress;
        #endregion

        #region Các hàm tạo
        public ProcessDES(frmMaHoaGiaiMa.ProgressEventArgs IncProg, frmMaHoaGiaiMa.ProgressInitHandler InitProg)
        {
        ...
        public void OnIncrementProgress(ProgressEventArgs e)
        {
            if (IncrementProgress != null)
                IncrementProgress(this, e);
        }

        public void OnInitProgress(ProgressInitArgs e)
        {
        ...
        public string FromDecimalToBinary(int binary)
        {
            if (binary < 0) 
            {
            ...
            string binarystring = "";
            int factor = 128; 
            for (int i = 0; i < 8; i++) { if (binary >= factor) 
                { 
        ...
        public byte FromBinaryToByte(string binary)
        {
            byte value = 0; 
            int factor = 128; 
            for (int i = 0; i < 8; i++)
        ...
        public string DoPermutation(string text, int[,] order)
        {
            ...
            // Bit đầu ghép bit cuối làm sốdòng (0-->3)
            ...
            // Lấy phần tửở mảng order, với giá trị từ0 đ ến 15, chuyển sang chuỗi nhị phân
        ...
        public string sBox_Transform(string text) 
        {
            ...
            for (int i = 0; i < 8; i++)      
            {
                ...
                // Mỗi lần lấy 6 bit// Chuyển thành 4 bit
